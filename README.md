...
- 第一步只提供四个随机排序的初步诊断候选项，单选后点击“确认诊断，继续作答”。本轮诊断锁定，不反馈对错。
- 第二步才从后端加载检查、治疗、临床处置选项。四栏顺序为诊断判断、检查、治疗、临床处置，每栏四项，后三栏多选。
- 点击“提交判断”后统一展示反馈、参考答案与依据。切换案例保留本页各案例的作答记录；完成后可重新作答。刷新网页会开始新的练习。
- 初始接口使用白名单，仅返回病例资料、匿名标识和诊断候选项。最终评估校验签名作答记录，防止直接跳过诊断或修改已锁定诊断；作答记录24小时过期，病例更新后也须重新开始。
- 默认单进程服务使用进程内签名密钥，重启服务后未完成的作答须重新开始。多实例或多 worker 部署应设置相同的高熵 `CASE_ATTEMPT_SECRET` 环境变量，避免跨实例验证失败。此流程用于教学练习，不是监考考试系统。
...
可用 `PLAYWRIGHT_MODULE` 指定 Playwright 模块路径、`CHROME_PATH` 指定浏览器、`CASE_TEST_OUTPUT` 指定截图目录。测试覆盖全部10个病例、诊断锁定、跨案例异步隔离、提交失败重试，以及1440/842/390像素布局。勿对生产服务运行此脚本，以免产生真实模型调用。
