...
3. **RAG 知识问答**：基于 180 个结构化知识块执行 BM25 与本地 TF-IDF 向量混合检索、加权 RRF 融合、模型 Reranker 重排序，并给出可点击核查的 `[K1]` 引用。
...
4. **答案与引用**：只把重排后的证据块交给回答模型，要求每个关键事实标注 `[K#]`。接口同时返回病种、文档、章节、原始 URL 和摘要，前端将其显示为可核查引用。
