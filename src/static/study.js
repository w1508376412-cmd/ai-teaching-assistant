...
window.Study = (() => {
  ...
  function dashboard() {
    ...
    let content;
    if (!session.pre_completed) content = `<span class="study-pill">首次使用 · 必须完成</span><h2>先独立完成前测</h2><p>请按当前掌握程度作答，不查阅资料或使用AI。20道单选题按基础、应用、综合递进；每道案例含4个单选小题。</p><p>建议预留30—40分钟。答案会自动保存，可以中断后继续；完成全部题目并提交即可进入学习，不要求达到及格分数。</p>${notice("前测暂不展示成绩和解析。完成后测后可查看两次成绩及逐题解析。提交后不能重做，请核对学号和姓名。")}${button("pre", "开始前测 →")}`;
  ...
  function questionMarkup(q, index, reviewMode = false) {
    ...
    return `<fieldset class="exam-question" id="exam-${q.id}" tabindex="-1"><legend><span class="question-number">${index}</span>${esc(q.stem)} <small>${q.points}分 · 单选</small></legend><div class="exam-options">${q.options.map((o, n) => `<label class="exam-option"><input type="radio" name="${q.id}" data-question="${q.id}" value="${o.id}" ${selected === o.id ? "checked" : ""} ${reviewMode ? "disabled" : ""}><span><b>${String.fromCharCode(65 + n)}</b>${esc(o.text)}</span></label>`).join("")}</div>${reviewMode ? `<div class="exam-explanation"><strong>${selected === q.correct ? "回答正确" : "需复习"} · 正确答案：${esc(q.options.find(o => o.id === q.correct)?.text)}</strong><p>${esc(q.explanation)}</p><small>知识点：${esc(q.point)} · ${q.sources.map(s => esc(s.document)).filter((v, i, a) => a.indexOf(v) === i).join("；")}</small></div>` : ""}</fieldset>`;
  }
